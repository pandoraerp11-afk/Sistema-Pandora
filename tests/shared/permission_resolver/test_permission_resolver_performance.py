"""Migrado de shared/tests/test_permission_resolver_performance.py.

Executado somente se PANDORA_PERF=1 estiver definido no ambiente.
"""
...
@pytest.mark.skipif(
    not os.environ.get("PANDORA_PERF"),
    reason="Set PANDORA_PERF=1 to run performance baseline tests",
)
def test_permission_resolver_baseline(settings):
    """Baseline simples de desempenho do permission_resolver (cold + warm)."""
    ...
    tenant = Tenant.objects.create(
        nome="Perf",
        slug="perf",
        enabled_modules={"modules": ["clientes", "financeiro", "obras"]},
    )
...
def test_menu_render_baseline(settings, django_template_rendered_signal):
    ...
    tpl.render(Context({"request": r}))  # warmup
