...
def test_get_current_tenant_auto_select_single_membership():
    ...
    # Sem tenant_id inicial, deve autoselecionar
    t = get_current_tenant(req)
    assert t == tenant
    # Rechamada faz cache (não consulta novamente) - validamos pelo atributo
...
def test_get_current_tenant_inactive_status():
    ...
    # Força sessão para inactive id
    req = _make_request(user)
    req.session["tenant_id"] = inactive.id
    t = get_current_tenant(req)
    assert t is None  # deve limpar porque status != active
