...
def test_precedence_deny_scoped_resource_over_allow_global():
    ...
    # Allow global genérico
    PermissaoPersonalizada.objects.create(user=user, concedida=True, acao="view", modulo="financeiro")
    # Allow global com recurso A
    PermissaoPersonalizada.objects.create(user=user, concedida=True, acao="view", modulo="financeiro", recurso="doc:A")
    # Deny scoped com mesmo recurso A deve prevalecer
    PermissaoPersonalizada.objects.create(
        user=user, concedida=False, acao="view", modulo="financeiro", recurso="doc:A", scope_tenant=tenant
    )
    ...
    assert allowed_B is True, reason_B  # cai no allow global (não há deny para B)
...
def test_precedence_expiration_filters_out_expired():
    ...
    TenantUser.objects.create(user=user, tenant=tenant)
    # Permissão deny expirada (não deve bloquear)
    PermissaoPersonalizada.objects.create(
        user=user,
        concedida=False,
        acao="view",
...
def test_role_fallback_after_no_personal_permissions():
    ...
    # Criar role com flag simulada
    role = Role.objects.create(tenant=tenant, name="ADMIN")
    # Simular atributo dinamicamente (role.can_view_cotacao)
    role.can_view_cotacao = True
    role.save()
    tu.role = role
...
def test_cache_invalidation_on_personal_permission_change():
    # Limpa cache para evitar interferência de chaves antigas (IDs podem ser reutilizados após flush do DB)
    ...
    # Grant allow
    PermissaoPersonalizada.objects.create(
        user=user, concedida=True, acao="view", modulo="financeiro", scope_tenant=tenant
    )
